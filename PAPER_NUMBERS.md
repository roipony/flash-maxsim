...
repo at `this repository/benchmarks/` and are re-derivable from
the matching `benchmarks/bench_*.py` script. **Two end-to-end retrieval JSONs
ship with this paper** instead of the code repo, because they were computed
from pre-existing institution-internal embedding files 
rather than from inputs the bench script downloads itself: see `data/` in this
paper repo. Specifically:
...
`benchmarks/bench_csr_vs_atomic.py` and its result JSONs at
...
## Table tab:colpali_ndcg — REAL-MM-RAG end-to-end retrieval parity (ColPali v1.2 + GVE)
...
and top-K overlap). Reproduction requires institution-internal multi-modal
