...
@triton.autotune(
    configs=[
        triton.Config({"BLOCK_Q": 16, "BLOCK_D": 64,  "DOCS_PER_CTA": 2}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_Q": 16, "BLOCK_D": 64,  "DOCS_PER_CTA": 4}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_Q": 32, "BLOCK_D": 64,  "DOCS_PER_CTA": 2}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_Q": 32, "BLOCK_D": 64,  "DOCS_PER_CTA": 4}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_Q": 32, "BLOCK_D": 128, "DOCS_PER_CTA": 2}, num_warps=8, num_stages=2),
        triton.Config({"BLOCK_Q": 32, "BLOCK_D": 128, "DOCS_PER_CTA": 4}, num_warps=8, num_stages=2),
        triton.Config({"BLOCK_Q": 64, "BLOCK_D": 64,  "DOCS_PER_CTA": 2}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_Q": 64, "BLOCK_D": 64,  "DOCS_PER_CTA": 4}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_Q": 64, "BLOCK_D": 128, "DOCS_PER_CTA": 2}, num_warps=8, num_stages=2),
        triton.Config({"BLOCK_Q": 64, "BLOCK_D": 128, "DOCS_PER_CTA": 4}, num_warps=8, num_stages=2),
    ],
    key=["Lq", "Ld", "d_pad"],
)
@triton.jit
def _maxsim_qreuse_kernel(
    ...
    BLOCK_Q: tl.constexpr, BLOCK_D: tl.constexpr, DOCS_PER_CTA: tl.constexpr,
):
    ...
    for q_start in range(0, Lq, BLOCK_Q):
        ...
        for d_start in range(0, Ld, BLOCK_D):
            ...
            if DOCS_PER_CTA >= 1:
                dv0 = d_off < len0
                D0  = tl.load(
                    D_ptr + tl.cast(doc0, tl.int64) * stride_d_b
                          + d_off[:, None] * stride_d_l + k_off[None, :] * stride_d_d,
                    mask=dv0[:, None] & k_mask[None, :], other=0.0,
                ).to(tl.float16)
            ...
            if DOCS_PER_CTA >= 2:
                dv1 = d_off < len1
                D1  = tl.load(
                    D_ptr + tl.cast(doc1, tl.int64) * stride_d_b
                          + d_off[:, None] * stride_d_l + k_off[None, :] * stride_d_d,
                    mask=dv1[:, None] & k_mask[None, :], other=0.0,
                ).to(tl.float16)
            ...
            if DOCS_PER_CTA >= 3:
                dv2 = d_off < len2
                D2  = tl.load(
                    D_ptr + tl.cast(doc2, tl.int64) * stride_d_b
                          + d_off[:, None] * stride_d_l + k_off[None, :] * stride_d_d,
                    mask=dv2[:, None] & k_mask[None, :], other=0.0,
                ).to(tl.float16)
            ...
            if DOCS_PER_CTA >= 4:
                dv3 = d_off < len3
                D3  = tl.load(
                    D_ptr + tl.cast(doc3, tl.int64) * stride_d_b
                          + d_off[:, None] * stride_d_l + k_off[None, :] * stride_d_d,
                    mask=dv3[:, None] & k_mask[None, :], other=0.0,
                ).to(tl.float16)
...
def _maxsim_splitk_phase1(
    Q_ptr, D_ptr, lengths_ptr, partial_ptr,
    B, num_splits,
    Lq: tl.constexpr, Ld: tl.constexpr, d: tl.constexpr, d_pad: tl.constexpr,
    stride_d_b, stride_d_l, stride_d_d,
    stride_q_l, stride_q_d,
    stride_p_b, stride_p_s, stride_p_q,
    ...
    for q_start in range(0, Lq, BLOCK_Q):
        ...
        tl.store(
            partial_ptr + doc_id64 * stride_p_b + split_id * stride_p_s + q_off * stride_p_q,
...
def _maxsim_splitk_phase2(
    partial_ptr, scores_ptr,
    B, num_splits: tl.constexpr, Lq: tl.constexpr,
    stride_p_b, stride_p_s, stride_p_q,
    ...
    for q_start in tl.static_range(0, Lq, BLOCK_Q):
        ...
        for s in tl.static_range(0, num_splits):
            pm = tl.load(
                partial_ptr + doc_id64 * stride_p_b + s * stride_p_s + q_off * stride_p_q,
                mask=q_valid, other=float("-inf"),
            ).to(tl.float32)
            m = tl.maximum(m, pm)
...
def flash_maxsim_splitk(
    ...
    D2 = D.contiguous()
    scores  = torch.empty(B, device=Q.device, dtype=torch.float32)
    partial = torch.full((B, num_splits, Lq), float("-inf"), device=Q.device, dtype=torch.float32)
