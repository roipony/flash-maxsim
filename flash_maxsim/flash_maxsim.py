...
def flash_maxsim_batched(Q: torch.Tensor, D: torch.Tensor, doc_lengths=None,
    ...
    if query_chunk_size is not None and Lq > query_chunk_size:
        ...
        q_lens_chunk = torch.full((Nq * num_chunks,), C, device=Q.device, dtype=torch.int32)
        if Lq % C != 0:
    ...
    if pad_lq and Lq > 0:
        ...
        if padded_Lq != Lq:
            if query_lengths is None:
                query_lengths = torch.full((Nq,), Lq, device=Q.device, dtype=torch.int32)
    ...
    scores, _ = _launch_fwd(Q2, D2, lengths, Nq, B, Lq, Ld, d, shared_docs, False, q_lengths=q_lens)
    return scores
...
# Re-export canonical naive from flash_maxsim.naive
from .naive import maxsim_naive  # noqa: F401
