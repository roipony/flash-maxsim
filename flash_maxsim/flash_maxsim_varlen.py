...
@triton.autotune(configs=_get_configs(), key=["max_Lq_bucket", "max_Ld_bucket", "d_pad"],
...
def _maxsim_varlen_kernel(
    ...
    for q_blk in tl.static_range(0, max_Lq_bucket, BLOCK_Q):
        ...
        for d_blk in range(0, max_Ld_bucket, BLOCK_D):
            ...
            D_block = tl.load(
                D_ptr + (d_start + d_off[:, None]) * stride_d_t + k_off[None, :] * stride_d_d,
...
def flash_maxsim_packed(
    ...
    if d < d_pad:
        ...
        Q_pad[:, :, :d] = Q_chunked
        D_pad = torch.zeros(D_packed.shape[0], d_pad, device=D_packed.device, dtype=D_packed.dtype)
        D_pad[:, :d] = D_packed
    else:
        Q_pad = Q_chunked
        D_pad = D_packed
...
def flash_maxsim_varlen(
    ...
    if d < d_pad:
        Q_pad = torch.zeros(Q_packed.shape[0], d_pad, device=Q_packed.device, dtype=Q_packed.dtype)
        Q_pad[:, :d] = Q_packed
        D_pad = torch.zeros(D_packed.shape[0], d_pad, device=D_packed.device, dtype=D_packed.dtype)
        D_pad[:, :d] = D_packed
    else:
